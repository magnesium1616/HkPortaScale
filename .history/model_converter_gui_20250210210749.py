...
class ModelConverterWindow(QMainWindow):
    ...
    def convert_model(self):
        ...
        try:
            ...
            param_path = f"{base_name}-raw.param"
            bin_path = f"{base_name}-raw.bin"
            ...
            # Step 3: NCNN最適化
            self.status_label.setText("NCNNモデル最適化中...")
            final_param = f"{base_name}.param"
            final_bin = f"{base_name}.bin"
            
            ncnnoptimize_path = os.path.join(self.ncnn_path, "ncnnoptimize.exe")
            subprocess.run([
                ncnnoptimize_path,
                param_path,
                bin_path,
            ...
            self.status_label.setText(f"変換完了: {os.path.basename(final_param)}")
