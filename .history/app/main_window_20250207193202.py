...
class UpscaleThread(QThread):
    ...
    def run(self):
        if os.path.isdir(self.args[0][0]):
            self.processor.process_directory(*self.args, **self.kwargs)
        else:
            self.processor.process_files(*self.args, **self.kwargs)
...
class MainWindow(QMainWindow):
    ...
    def _on_upscale_clicked(self):
        ...
        self.upscale_thread = UpscaleThread(
            ...
            output_settings["output_dir"] if len(self.input_paths) > 1 else None
        )
